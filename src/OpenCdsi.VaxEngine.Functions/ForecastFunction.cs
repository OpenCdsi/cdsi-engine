...
/// <summary>
/// The same forecast capability OpenCdsi.VaxEngine.Api exposes over HTTP, as a second API surface. Reuses
/// OpenCdsi.VaxEngine.Contracts' exact same request/response DTOs and mapping - no duplicated shape logic
...
/// about adding API key auth to OpenCdsi.VaxEngine.Api: Functions gets a form of this for free, OpenCdsi.VaxEngine.Api
...
public class ForecastFunction(ReferenceDataRepository data, ILogger<ForecastFunction> logger)
{
        ...
        [HttpTrigger(AuthorizationLevel.Function, "post", Route = "api/v1/forecast")] HttpRequest req)
    {
        ...
        {
            // Same failure mode OpenCdsi.VaxEngine.Api's own BadHttpRequestException handling covers (missing
        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "health")] HttpRequest req)
    {
        ...
        // without a key, same reasoning OpenCdsi.VaxEngine.Api's own /health endpoint follows.
